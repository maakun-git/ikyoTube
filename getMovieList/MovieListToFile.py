## Youtubeの動画リストをファイルに書き出すとともにターミナルにも出力する
...
search_response = youtube.search().list(
    ...
    publishedBefore="2022-09-15T00:00:00Z"
).execute()
...
with open('Movielist.txt', 'a', encoding='utf-8', newline="", ) as file:
    writer = csv.writer(file, delimiter=';')

    for item in search_response["items"]:
        total = total + 1
        if item["id"]["kind"] == 'youtube#video':
            writer.writerow([item["snippet"]["publishedAt"],  "https://www.youtube.com/watch?v=" + item["id"]["videoId"],  item["snippet"]["title"]])
